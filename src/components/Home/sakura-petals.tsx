...
export function SakuraPetals() {
  // Generate 60 petals with random properties
  const petals = useMemo<Petal[]>(() => {
    return Array.from({ length: 60 }, (_, i) => ({
      ...
      depth: Math.random(), // 0-1, for layering and opacity
...
function Petal({ startX, startY, delay, duration, size, depth, rotation }: Petal) {
  // Create curved path with bezier-like motion
  const pathVariants = {
    initial: {
      x: `${startX}vw`,
      y: `${startY}vh`,
      ...
      <motion.div variants={wobbleVariants} animate="animate">
        ...
        >
          ...
          <ellipse
            ...
            rx="2"
            ry="3"
            fill={`rgba(255, 230, 240, ${0.4 + depth * 0.3})`}
          />
        </svg>
      </motion.div>
    </motion.div>
